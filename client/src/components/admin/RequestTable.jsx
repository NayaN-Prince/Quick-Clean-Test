...
const RequestTable = () => {
        ...
        const fetchRequests = async () => {
            try {
                const { data, error } = await supabase
                    ...
                    .select(`
                        *,
                        customer:user_id (name, email, mobile_contact),
    ...
    const filteredRequests = requests.filter(req => {
        ...
        const matchesSearch =
            ...
            req.garbage_type?.toLowerCase().includes(searchTerm.toLowerCase());
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {filteredRequests.map((req) => (
                                <tr key={req.id} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-6 py-5">
                                        <div className="flex flex-col">
                                            <span className="text-sm font-semibold text-slate-700">{req.customer?.name}</span>
                                            <span className="text-[11px] text-slate-400">{req.mobile_contact || req.customer?.email}</span>
                                        </div>
                                    </td>
                                    <td className="px-6 py-5">
                                        {!req.worker_id ? (
